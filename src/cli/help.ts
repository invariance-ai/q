...
const TOPICS: Record<string, () => string> = {
  ...
  keys: () =>
    [
      chalk.bold("Providing API keys (any provider)"),
      "",
      "The provider is inferred from the model id, and q reads that provider's key",
      "from the environment first, then the config file. Supported out of the box:",
      "",
      "  openai      gpt-*, o*           OPENAI_API_KEY",
      "  anthropic   claude-*            ANTHROPIC_API_KEY",
      "  xai         grok-*              XAI_API_KEY",
      "  google      gemini-*            GEMINI_API_KEY (or GOOGLE_API_KEY)",
      "  perplexity  sonar*, pplx*       PERPLEXITY_API_KEY",
      "  groq        groq/<model>        GROQ_API_KEY",
      "  mistral     mistral-*           MISTRAL_API_KEY",
      "  deepseek    deepseek-*          DEEPSEEK_API_KEY",
      "  openrouter  openrouter/<model>  OPENROUTER_API_KEY   (universal catch-all)",
      "",
      "Pick a model with `q model set <id>` or per-call `-m <id>`. For anything",
      "whose name doesn't prefix-match, use a provider prefix, e.g.",
      "  q -m openrouter/anthropic/claude-3.5-sonnet what changed in this repo",
      "",
      "Config file alternative: q config set keys.<provider> <key>",
      "Env-sourced keys are never written to disk; `q config list/get` redact secrets.",
    ].join("\n"),
  telemetry: () =>
    [
